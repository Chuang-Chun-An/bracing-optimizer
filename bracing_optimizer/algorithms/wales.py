...
def classify_length(length: int, cfg: Config) -> str:
    ...
    - long: 8000 < len <= 10000
    """
    return classify_material_length(length, _material_length_rules(cfg))


def _material_length_rules(cfg: Config) -> MaterialLengthRules:
    """Adapt legacy Waler config fields to the shared material policy."""

    return MaterialLengthRules(
        short_min=cfg.short_segment_min,
        short_max=cfg.short_segment_max,
        mid_min=cfg.mid_segment_min,
        mid_max=cfg.mid_segment_max,
        long_min=cfg.long_segment_min,
        long_max=cfg.long_segment_max,
    )
...
def segment_ratio_summary(
    ...
    return analysis.counts, analysis.ratios
...
def calculate_ratio_penalty(
    ...
    return analysis.penalty, analysis.ratios
