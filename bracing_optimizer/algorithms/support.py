...
def clone_plan_with_support_id(
    ...
    shared_layout_group: Optional[str] = None,
) -> SupportPlan:
    return SupportPlan(
        ...
        shared_layout_group=(
            str(getattr(plan, "shared_layout_group", "") or "")
            if shared_layout_group is None
            else str(shared_layout_group or "")
        ),
    )
...
def material_style_from_lengths(
    ...
    for length in normalized_lengths:
        category = classify_length(int(length))
...
def shared_layout_signature(plan: SupportPlan) -> Tuple[Tuple[str, int], ...]:
    """Ordered physical layout that every member of a double support shares."""

    return tuple(
        (str(kind).lower(), int(length))
        for kind, length in getattr(plan, "pieces", []) or []
    )


def plans_share_layout_group(first: SupportPlan, second: SupportPlan) -> bool:
    first_group = str(getattr(first, "shared_layout_group", "") or "").strip()
    second_group = str(getattr(second, "shared_layout_group", "") or "").strip()
    return bool(first_group) and first_group == second_group


def shared_layout_groups_valid(plans: List[SupportPlan]) -> bool:
    signatures: Dict[str, Tuple[Tuple[str, int], ...]] = {}
    member_counts: Counter[str] = Counter()
    for plan in plans:
        group_id = str(getattr(plan, "shared_layout_group", "") or "").strip()
        if not group_id:
            continue
...
def normalize_material_ratio_targets(
    ...
    return MaterialRatioTargets.normalized(
        short_ratio,
        mid_ratio,
        long_ratio,
    ).as_dict()
...
def calculate_material_ratio_analysis(
    ...
    analysis = analyze_material_ratios(
        steel_lengths,
        MaterialRatioTargets.from_mapping(targets),
        weight=weight,
    )
    return {
        ...
        "targets": targets,
        "ratio_deviation": analysis.ratio_deviation,
        "penalty": analysis.penalty,
        "weight": analysis.weight,
        "classified_total": analysis.classified_total,
...
def make_global_solution(
    ...
    *,
    ...
    solution_valid = (
        ...
        and shared_layout_groups_valid(plans)
        if valid is None
        else bool(valid) and shared_layout_groups_valid(plans)
    )
