...
from bracing_optimizer.domain.project_domain import ProjectDomainModel
from bracing_optimizer.application.project_mapper import ProjectRowMapper
...
class ProjectDataModel:
    ...
    def replace_row(
        ...
        """Normalize and replace one row while preserving the owned table list."""

        rows = self.rows(table_name)
        if not 0 <= index < len(rows):
            raise IndexError(f"{table_name} 資料列索引超出範圍：{index}")
        normalized = normalize_project_row(table_name, row)
    ...
    def to_domain(self, *, strict: bool = True) -> ProjectDomainModel:
        return ProjectRowMapper.project(
            walers=self.walers,
            struts=self.struts,
            braces=self.braces,
            strict=strict,
