...
DEFAULT_JACK_ASSET = (
    Path(__file__).resolve().parent / "assets" / "dxf" / "jack_symbol.dxf"
)
    ...
    for role, converted_key, id_field, rows in specs:
        ...
        for row_index, (member_id, row_line) in enumerate(row_items):
            ...
            if candidate_index is None:
                exact_index = candidate_indexes_by_id.get(member_id)
                if exact_index is not None and exact_index not in used_candidates:
                    candidate_index = exact_index
                    used_candidates.add(exact_index)
            if candidate_index is None:
                continue
